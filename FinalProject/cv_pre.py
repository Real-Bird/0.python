...
def tn_detector(frame, network, model):
    # 프레임 크기
    (h, w) = frame.shape[:2]
...
while True:
    ...
    # 자세 인식 및 tn Detector 수행
    (locations, predicts) = tn_detector(frame, network, model)
    ...
    for (box, predict) in zip(locations, predicts):
        ...
        # bounding box 레이블 설정
        label = "co" if co < fo else "fo"
        
        # bounding box 색상 설정
        if label == "co" and max(co, fo) * 100 <= 70:
            color = (0, 255, 0) # 초록
        elif label == "fo" and max(co, fo) * 100 <= 70:
            color = (0, 0, 255) # 빨강
        else:
            color = (0, 255, 255) # 노랑

        # 확률 설정
        label = "{}: {:.2f}%".format(label, max(co, fo) * 100)
